...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filter (blockWithHistory => !explored.contains(blockWithHistory._1))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moves) #:: xs => (block, moves) #:: from(xs ++ newNeighborsOnly(neighborsWithHistory(block, moves), explored), explored + block)
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter (blockWithMoves => done(blockWithMoves._1))
