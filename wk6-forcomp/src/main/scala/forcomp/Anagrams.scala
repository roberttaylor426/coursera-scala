...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val occMap = w.toLowerCase().groupBy(identity) map { case (letter, occs) => (letter, occs.length)}
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    for ((c, i) <- x; if (i > yMap(c))) yield (c, i - yMap(c))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def anagramsForOccurrences(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(Nil)
      case x :: xs =>
        for (
          occs <- combinations(occurrences);
          word <- dictionaryByOccurrences(occs);
          subSentence <- anagramsForOccurrences(subtract(occurrences, occs))
